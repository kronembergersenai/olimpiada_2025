...
// VERSÃO CORRIGIDA - SISTEMA DE LINKS DIRETOS
// ============================================================
        ...
        if (response.ok) {
            ...
            for (const modalidade in arquivosCompartilhados) {
                if (arquivosCompartilhados.hasOwnProperty(modalidade)) {
                    ...
                    // Remover duplicatas baseado no nome e link
            ...
            console.log('arquivos.json não encontrado. Usando apenas arquivos locais.');
            return false;
        }
    } catch (error) {
        console.log('Usando apenas arquivos locais:', error);
        return false;
    }
}

// Dados das etapas de conhecimento (mantendo o mesmo)
...
// Cronograma por dias (mantendo o mesmo)
...
// Funções de avaliação (mantendo as mesmas)
...
// ============================================================
// ESTATÍSTICAS E GRÁFICO (mantendo o mesmo)
// ============================================================
...
// ============================================================
// PROCEDIMENTOS TÉCNICOS (mantendo o mesmo)
// ============================================================
...
// ============================================================
// NOVA FUNCIONALIDADE DE ARQUIVOS - SIMPLIFICADA E ROBUSTA
// ============================================================
    ...
    if (arquivos.length === 0) {
        container.innerHTML = `
            <div class="no-arquivos">
                <p><i class="fas fa-folder-open"></i> Nenhum arquivo adicionado ainda.</p>
                <p class="help-text">Adicione links para documentos, vídeos ou outros materiais de estudo.</p>
            </div>
        `;
    } else {
...
function criarItemArquivo(arquivo, modalidade) {
    // Determinar tipo de arquivo baseado no link
    let icone = 'fas fa-link';
    let tipo = 'Link';
    
    if (arquivo.link.includes('drive.google.com')) {
        icone = 'fab fa-google-drive';
        tipo = 'Google Drive';
    } else if (arquivo.link.includes('youtube.com') || arquivo.link.includes('youtu.be')) {
        icone = 'fab fa-youtube';
        tipo = 'YouTube';
    } else if (arquivo.link.includes('.pdf')) {
        icone = 'fas fa-file-pdf';
        tipo = 'PDF';
    } else if (arquivo.link.includes('.doc') || arquivo.link.includes('.docx')) {
        icone = 'fas fa-file-word';
        tipo = 'Word';
    } else if (arquivo.link.includes('.ppt') || arquivo.link.includes('.pptx')) {
    ...
    return `
        <div class="arquivo-item" data-arquivo-id="${arquivo.id}">
            <div class="arquivo-info">
                <div class="arquivo-icon-container">
                    <i class="${icone} arquivo-icon"></i>
                    <span class="arquivo-tipo">${tipo}</span>
                </div>
                <div class="arquivo-details">
                    <h5>${arquivo.nome}</h5>
                    <p>Adicionado em ${new Date(arquivo.dataAdicao).toLocaleDateString('pt-BR')}</p>
                    <p class="arquivo-link">${arquivo.link}</p>
                </div>
            </div>
            <div class="arquivo-acoes">
                <button class="arquivo-btn abrir" onclick="abrirArquivo('${arquivo.link}')" title="Abrir em nova aba">
                    <i class="fas fa-external-link-alt"></i>
                    Abrir
                </button>
                <button class="arquivo-btn copiar" onclick="copiarLink('${arquivo.link}')" title="Copiar link">
                    <i class="fas fa-copy"></i>
                    Copiar
                </button>
                <button class="arquivo-btn remover" onclick="confirmarRemocao(${arquivo.id}, '${modalidade}')" title="Remover arquivo">
                    <i class="fas fa-trash"></i>
                    Remover
                </button>
            </div>
...
// Função para adicionar arquivo com validação melhorada
function adicionarArquivo(modalidade) {
    ...
    if (!link) {
        alert('Por favor, insira o link do arquivo.');
        linkInput.focus();
        return;
    }
    ...
    if (arquivosExistentes.some(arq => arq.nome === nome)) {
        alert('Já existe um arquivo com este nome. Por favor, escolha outro nome.');
        nomeInput.focus();
        return;
    }
    ...
    mostrarMensagemSucesso(`Arquivo "${nome}" adicionado com sucesso!`);
}
...
function copiarLink(link) {
    navigator.clipboard.writeText(link).then(function() {
        mostrarMensagemSucesso('Link copiado para a área de transferência!');
    }).catch(function(err) {
...
// Função para confirmar remoção de arquivo
function confirmarRemocao(id, modalidade) {
    const arquivo = estadoApp.arquivos[modalidade].find(arq => arq.id === id);
    if (!arquivo) return;
    
    if (confirm(`Tem certeza que deseja remover o arquivo "${arquivo.nome}"?\n\nEsta ação não pode ser desfeita.`)) {
        removerArquivo(id, modalidade);
    }
}

// Função para remover arquivo
function removerArquivo(id, modalidade) {
    const arquivoIndex = estadoApp.arquivos[modalidade].findIndex(arq => arq.id === id);
    if (arquivoIndex === -1) {
        alert('Arquivo não encontrado.');
        return;
    }
    
    const nomeArquivo = estadoApp.arquivos[modalidade][arquivoIndex].nome;
    estadoApp.arquivos[modalidade].splice(arquivoIndex, 1);
    
    salvarEstado();
    carregarArquivosModalidade(modalidade);
    
    mostrarMensagemSucesso(`Arquivo "${nomeArquivo}" removido com sucesso!`);
}
...
async function carregarEstado() {
    try {
        ...
        if (estadoSalvo) {
            ...
            // Para arquivos, fazer merge com os compartilhados
...
document.addEventListener('keydown', (e) => {
    if (e.key === 'Escape') {
        const modalsAbertos = document.querySelectorAll('.modal:not(.hidden)');
        modalsAbertos.forEach(modal => modal.classList.add('hidden'));
    }
});
